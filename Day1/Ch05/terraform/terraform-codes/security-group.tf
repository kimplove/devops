...
resource "aws_security_group" "test-sg-eks-cluster" {
  name        = "sktwykim-sg-eks-cluster"
  description = "security_group for sktwykim-eks-cluster"
...
resource "aws_security_group_rule" "test-sg-eks-cluster-ingress" {
  ...
  type              = "ingress"
  description       = "ingress security_group_rule for sktwykim-eks-cluster"
  from_port         = 0
  to_port           = 0
  protocol          = "-1"
...
resource "aws_security_group_rule" "test-sg-eks-cluster-egress" {
  ...
  security_group_id = aws_security_group.test-sg-eks-cluster.id
  type              = "egress"
  description       = "egress security_group_rule for sktwykim-eks-cluster"
  from_port         = 0
  to_port           = 0
  protocol          = "-1"
  cidr_blocks       = ["0.0.0.0/0"]
